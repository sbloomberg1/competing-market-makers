...
2. All makers commit replacement quotes before any current quote is revealed.
3. Fair value moves; an informed taker trades first, followed by retail.
4. Repeat for 24 ticks. There are no information purchases in this version.
...
Each player receives `total signed edge / (N × 10000)`. The greatest signed score among valid players wins the game. An exact tie or all-player forfeit returns winner `-1`. A forfeiting player receives `-1000`, below the bounded score of every valid policy, and cannot win. The platform runs the bracket; its four-player advancement contract is awaiting confirmation. A relative winner can still have negative edge.
...
The proposed evaluation uses **8,192 episodes**, batches of up to **256**, and a **100 ms deadline for the entire batch response**. All episodes have 24 ticks. The platform-owned input can select a 256-episode smoke test; miners do not choose evaluation size. One CPU and 512 MiB are allocated per sandbox.
...
docker build --platform linux/amd64 -f referee/Dockerfile -t cmm-referee:dev .
python3 scripts/docker_match.py --referee-image cmm-referee:dev --submission player/submission.py --submission benchmarks/wide.py --submission benchmarks/tight.py --submission benchmarks/static.py --episodes 256 --output evidence/my-run
